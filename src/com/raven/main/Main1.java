...
import static com.raven.projet_java_s2.abonne_service.chercher_black_listed;
import com.raven.service.ServiceUser;
import static com.raven.service.ServiceUser.nbre_de_fois;
import static com.raven.service.ServiceUser.role;
   ...
   private void register()
   {
    ...
    try {
        if (service.checkDuplicateUser(user.getUserName())) {
            ...
            showMessage(Message.MessageType.ERROR, "Password must contain at least 4 letters and one special character");
        } else {
            service.insertUser(user);
            showMessage(Message.MessageType.SUCCESS, "Welcome");
        }
...
private boolean isValidPassword(String password) {
    ...
    private void login() {
        ...
        try {
            ...
            if (user != null) {
                this.dispose();
                if (role.equals("abonne"))
                {
                    if (nbre_de_fois==1)
                    {
                        InterfacePourNouvelAbonne a=new InterfacePourNouvelAbonne();
                        a.setVisible(true);
                    }
                    else
                    {
                        if(chercher_black_listed(userID))
                        {
                            InterfaceBlackList i=new InterfaceBlackList();
                            i.setVisible(true);
                        }
                        else
                        {
